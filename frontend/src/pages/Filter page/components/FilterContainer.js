...
import { Box, TextField, FormControlLabel, Checkbox, Slider, Typography, Button } from "@mui/material";
  ...
  return (
    <Box p={2} className="filter-container">
      ...
      {genres.map((genre) => (
        <FormControlLabel
          key={genre}
          control={<Checkbox checked={selectedGenres.includes(genre)} onChange={() => handleGenreChange(genre)} />}
          label={genre}
        />
      ))}
      ...
      {platforms.map((platform) => (
        <FormControlLabel
          key={platform}
          control={<Checkbox checked={selectedPlatforms.includes(platform)} onChange={() => handlePlatformChange(platform)} />}
          label={platform}
        />
      ))}
